...
def generate_launch_description():
    xsmrobot_bringup_dir = get_package_share_directory(
        'xsmrobot_bringup')
    ...
    urdf2tf = launch.actions.IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            [xsmrobot_bringup_dir, '/launch', '/urdf2tf.launch.py']),
    )

    odom2tf = launch_ros.actions.Node(
        package='xsmrobot_bringup',
        executable='odom2tf',
        output='screen'
    )
    ...
    # 修复1：启动前强制删除残留的虚拟串口文件，避免冲突导致雷达关闭
    remove_old_tty = launch.actions.ExecuteProcess(
        cmd=['rm', '-f', '/tmp/tty_laser'],
        output='screen'
    )
